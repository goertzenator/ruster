...
impl<'a, 'b> From<Binder<'a, 'b, &'a [Term<'a>]>> for Term<'b> {
	fn from(b: Binder<'a, 'b, &'a [Term<'a>]>) -> Self {
		Term::new(
			unsafe {
...
// convert Term to &[Term]
impl<'a, 'b> TryFrom<Binder<'a, 'b, Term<'a>>> for &'b [Term<'b>] {
	type Err = Error;
...
macro_rules! impl_tuple_conversion {
    ($arity:expr, [$($indices:tt),*], [$($typevars:ident),*])  => (
    	...
    	impl<'a, 'b, $($typevars),*> From<Binder<'a, 'b, ($($typevars),*,)>> for [Term<'b>;$arity]
    		where
    		$(Binder<'a, 'b, $typevars> : Into<Term<'b>>),*
    	{
    		fn from(b: Binder<'a, 'b, ($($typevars),*,)>) -> Self {
    			let tup = b.val;
    			let env = b.env;
    	...
    	impl<'a, 'b, $($typevars),*> From<Binder<'a, 'b, ($($typevars),*,)>> for Term<'b>
    		where
    		$(Binder<'a, 'b, $typevars> : Into<Term<'b>>),*
    	{
    		fn from(b: Binder<'a, 'b, ($($typevars),*,)>) -> Self {
...
//				let terms:[Term; $arity] = [$(tup.$indices.bind(env).into()),*];
    	...
    	impl<'a, 'b, $($typevars),*> TryFrom<Binder<'a, 'b, &'a [Term<'a>]>> for ($($typevars),*,)
    		where
    		$($typevars: TryFrom<Binder<'a, 'b, Term<'a>>, Err = Error>),*
    	{
    		type Err = Error;
    		fn try_from(b: Binder<'a, 'b, &'a [Term<'a>]>) -> Result<Self>
    		{
    			let terms = *b.val;
    			let env = b.env;
    	...
    	impl<'a, 'b, $($typevars),*> TryFrom<Binder<'a, 'b, Term<'a>>> for ($($typevars),*,)
    		where
    		$($typevars: TryFrom<Binder<'a, 'b, Term<'a>>, Err = Error>),*
    	{
    		type Err = Error;
    		fn try_from(b: Binder<'a, 'b, Term<'a>>) -> Result<Self> {
    			let term = *b.val;
    			let env = b.env;
...
//
impl<'a, 'b> From<Binder<'a, 'b, ()>> for [Term<'b>; 0]
{
    fn from(_b: Binder<'a, 'b, ()>) -> Self {
        []
    }
}

impl<'a, 'b> From<Binder<'a, 'b, ()>> for Term<'b>
{
    fn from(b: Binder<'a, 'b, ()>) -> Self {
        let env = b.env;
        let terms: [Term; 0] = [];
        terms.as_ref().bind(env).into()
    }
...
impl<'a, 'b> TryFrom<Binder<'a, 'b, &'a [Term<'a>]>> for ()
{
    type Err = Error;
    fn try_from(b: Binder<'a, 'b, &'a [Term<'a>]>) -> Result<Self> {
        let terms = *b.val;
...
impl<'a, 'b> TryFrom<Binder<'a, 'b, Term<'a>>> for ()
{
    type Err = Error;
    fn try_from(b: Binder<'a, 'b, Term<'a>>) -> Result<Self> {
        let term = *b.val;
